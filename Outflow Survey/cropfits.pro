...
;  
; :Parameters
;  input: a string of an input file name or a list of dat and hdr
;     varibles.
;  
...
;     
;  output: a string of the output file name  or a struct of a dat and hdr
;     Specify a user defined output file name or a struct name.
;     
;  /dataform
;     Use it when cropping data with hdr directly.
;  
;     
;     
; :Examples
...
; :History
;  v2.0 2015-12-5
;   Add new features. Now it is convenient to crop a data cube
;   with a header. Use /datalist instead.
;   
;  v1.6 2015-1-30
...
pro cropfits,input,range1,range2,range3,dim=dim,pix=pix,output=output,dataform=dataform
...
if n_params() lt 2 then begin
    print, '************************************************************************'
    print, '  Syntax - CROPFITS, fitsfile[or list(dat,hdr)], range1[, range2, range3]' 
    print, '            [,dim=''x[y,v,xy,xv,yv,xyv]'', /pix, output=output[filename
    print, '            or structname],/dataform]'
    print, '************************************************************************'
...
if keyword_set(dataform) && size(input,/type) ne 11 then begin
    print, 'ERROR: input data is not a list!'
    return
endif

if ~keyword_set(dataform) then begin
  if ~file_test(input) then begin 
    print, 'ERROR: input fits file does not exist!'
    return
  endif
endif

if ~keyword_set(output) && ~keyword_set(dataform) then begin
    output=file_basename(input,'.fits',/fold_case)
endif
...
if keyword_set(dataform) then begin
  ...
  dat=input[0]
  hdr=input[1]
endif else fits_read,input,dat,hdr
sz=size(dat)
...
if keyword_set(dataform) then begin
  mkhdr,chdr,dat
  sxaddpar,hdr,'NAXIS1',sxpar(chdr,'NAXIS1')
  sxaddpar,hdr,'NAXIS2',sxpar(chdr,'NAXIS2')
  if  n_elements(sxpar(chdr,'NAXIS*')) eq 3 then sxaddpar,hdr,'NAXIS3',sxpar(chdr,'NAXIS3')
  output={dat:dat,hdr:hdr}
  return
endif else fits_write,output+'_C.fits',dat,hdr
